...
- *Node.js* - Node is used for kik Social's streaming API and other platform services
...
kik Social requires a standard non-root user account for day-to-day operations and work. This can be your own account or (if following this document for the first time) the `kiksocial` user.

Creating a `kiksocial` user is simple and can make following the rest of this guide very simple.

```
adduser --disabled-password --quiet kiksocial
```
...
The public-facing Web service `kiksocial-web` is currently built using Ruby On Rails. A developer workstation user account, therefore, must configure [`rbenv`](https://github.com/rbenv/rbenv) and [`ruby-build`](https://github.com/rbenv/ruby-build) as follows:
...
```sh
# By convention at kik, we work in ~/projects
mkdir -p ~/projects
cd ~/projects
...
# To setup the `kiksocial_development` database, run:
...
# You can then run kik Social with:
bundle exec rails server
```

## Managing your development environment

It is assumed that development hosts are not publicly accessible. For best security, there should be no route from a public network to your kik Social development workstation.
