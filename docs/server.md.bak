...
It is assumed that you have technical knowledge and skills sufficient to administer Linux servers and scale them, if necessary. kik Social communities that succeed have *no upper limit* to the maximum amount of users, posts, attachments, and problems related to operating publicly-accessible online communities at scale. Fortunately, kik Social does not require you to operate at scale. You can run this server in single-user mode, which guarantees users complete ownership of all their data while still participating in the kik Platform.
...
>
>
> Not only will this help you not lose your place if you are disconnected, it will let you have multiple terminal windows open for switching contexts (root user versus the kiksocial user).
>
...
- libprotobuf-dev and protobuf-compiler - kik Social uses these for language detection
- nginx - nginx is our frontend web server
- redis-* - kik Social uses redis for its in-memory data structure store
- postgresql-* - kik Social uses PostgreSQL as its SQL database
- nodejs - Node is used for kik Social's streaming API and other platform services
...
```sh
adduser --disabled-password kiksocial
```

Log in as the `kiksocial` user:
...
[Node Version Manager](https://github.com/nvm-sh/nvm) is a tool used for managing Node.js deployments. By convention at kik, we only use Node.js as a standard user. No part of Node.js is managed or executed with superuser privileges. Those responsibilities are handled by Nginx later in this document.
...
```sh
# Return to kiksocial user's home directory
cd ~
...
That is all we need to do for now with the `kiksocial` user, you can now `exit` back to root.
...
Eventually, the Ruby On Rails dependencies are going away. This is a stop-gap solution while we continue our migration to a kik-native implementation of ActivityPub/GNU Social on HYDRA.
...
CREATE USER kiksocial CREATEDB;
\q
```
...
server {
  ...
  root /home/kiksocial/live/public;
...
**Make sure to replace any occurrence of 'example.com' with your kik Social instance's domain.**
...
## kik Social Application Configuration

We will configure the kik Social application.

For this we will switch to the `kiksocial` system user:
...
```sh
cd ~/live
RAILS_ENV=production bundle exec rake kiksocial:setup
```
...
## kik Social systemd Service Files

We will need three [systemd](https://github.com/systemd/systemd) service files for each kik Social service.
...
ExecStart=/home/kiksocial/.rbenv/shims/bundle exec sidekiq -c 5 -q default -q push -q mailers -q pull
TimeoutSec=15
Restart=always
...
```
[Unit]
Description=kiksocial-streaming
After=network.target
...
```sh
systemctl enable /etc/systemd/system/kiksocial-*.service
```

Now start the services:

```sh
systemctl start kiksocial-*.service
```

Check that they are properly running:

```sh
systemctl status kiksocial-*.service
```

## Remote media attachment cache cleanup

kik Social downloads media attachments from other instances and caches it locally for viewing. This cache can grow quite large if not cleaned up periodically and can cause issues such as low disk space or a bloated S3 bucket.

The recommended method to clean up the remote media cache is a cron job that runs daily like so (put this in the kiksocial system user's crontab with `crontab -e`.)

```sh
RAILS_ENV=production
@daily cd /home/kiksocial/live && /home/kiksocial/.rbenv/shims/bundle exec rake kiksocial:media:remove_remote
```
...
SMTP_PORT=587
SMTP_LOGIN=anAccountThatIsntPostmaster@kiksocial.domain.com
