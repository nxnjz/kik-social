...
namespace :kiksocial do
  ...
  task :setup do
    ...
    begin
      ...
      using_docker        = prompt.yes?('Are you using Docker to run kik Social?')
      ...
      loop do
        ...
        env['DB_NAME'] = prompt.ask('Name of PostgreSQL database:') do |q|
          q.required true
          q.default using_docker ? 'postgres' : 'kiksocial_production'
          q.modify :strip
        end

        env['DB_USER'] = prompt.ask('Name of PostgreSQL user:') do |q|
          q.required true
          q.default using_docker ? 'postgres' : 'kiksocial'
          q.modify :strip
        end
      ...
      if prompt.yes?('Save configuration?')
        ...
        File.write(Rails.root.join('.env.production'), "# Generated with kiksocial:setup on #{Time.now.utc}\n\n" + env.each_pair.map { |key, value| "#{key}=#{value}" }.join("\n") + "\n")
        ...
        prompt.say "\n"
        prompt.ok 'All done! You can now power on the kik Social server.'
        prompt.say "\n"
