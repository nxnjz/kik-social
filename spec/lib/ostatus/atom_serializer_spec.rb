...
RSpec.describe OStatus::AtomSerializer do
  ...
  shared_examples 'namespaces' do
    it 'adds namespaces' do
      ...
      expect(element['xmlns:kiksocial']).to eq OStatus::TagManager::MTDN_XMLNS
    end
  end
  ...
  describe '#author' do
    ...
    it "appends kiksocial:scope element with 'private' if locked" do
      account = Fabricate(:account, locked: true)

      author = OStatus::AtomSerializer.new.author(account)

      scope = author.nodes.find { |node| node.name == 'kiksocial:scope' }
      expect(scope.text).to eq 'private'
    end

    it "appends kiksocial:scope element with 'public' if unlocked" do
  ...
  describe '#entry' do
    ...
    context 'if status is present' do
      ...
      it 'appends kiksocial:scope element with visibility' do
        status = Fabricate(:status, visibility: :public)

        entry = OStatus::AtomSerializer.new.entry(status.stream_entry)

        scope = entry.nodes.find { |node| node.name == 'kiksocial:scope' }
        expect(scope.text).to eq 'public'
      end
