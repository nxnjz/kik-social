...
describe AccountFinderConcern do
  ...
  describe 'remote finders' do
    before do
      @account = Fabricate(:account, username: 'Alice', domain: 'kik.com')
    end

    describe '.find_remote' do
      it 'returns exact match result' do
        expect(Account.find_remote('alice', 'kik.com')).to eq(@account)
      end
      ...
      it 'returns nil when username does not match' do
        expect(Account.find_remote('a_ice', 'kik.com')).to be_nil
      end
    ...
    describe '.find_remote!' do
      ...
      it 'raises on non-matching result' do
        expect { Account.find_remote!('missing', 'kik.host') }.to raise_error(ActiveRecord::RecordNotFound)
      end
