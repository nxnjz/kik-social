...
RSpec.describe User, type: :model do
  ...
  describe 'whitelist' do
    around(:each) do |example|
      ...
      Rails.configuration.x.email_domains_whitelist = 'kik.space'
    ...
    it 'should allow a user to be created if they are whitelisted' do
      user = User.new(email: 'foo@kik.space', account: account, password: password, agreement: true)
      expect(user.valid?).to be_truthy
    end

    it 'should not allow a user with a whitelisted top domain as subdomain in their email address to be created' do
      user = User.new(email: 'foo@kik.space.userdomain.com', account: account, password: password, agreement: true)
      expect(user.valid?).to be_falsey
    end
    ...
    context do
      ...
      it 'should not allow a user to be created with a specific blacklisted subdomain even if the top domain is whitelisted' do
        Rails.configuration.x.email_domains_blacklist = 'blacklisted.kik.space'

        user = User.new(email: 'foo@blacklisted.kik.space', account: account, password: password)
        expect(user.valid?).to be_falsey
      end
    end
  end
  ...
  it_behaves_like 'Settings-extended' do
    def create!
      User.create!(account: Fabricate(:account), email: 'foo@kik.space', password: 'abcd1234', agreement: true)
    end
